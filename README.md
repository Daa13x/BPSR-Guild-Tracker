...
In `github` mode the canonical values live in each `state/members/<publicMemberId>.json` file; the Players columns `NMRaidComplete`/`NMRaidDate` and `EHRaidComplete`/`EHRaidDate` are populated only by the explicit one-way GitHub → Sheets mirror sync. The legacy `RaidComplete`/`RaidDate` columns are kept only for the one-time Sheets-to-GitHub migration: a completed legacy Raid becomes `easyHardRaidCompleted: true` (incomplete → `false`), `nmRaidCompleted` defaults to `false`, and the legacy column is cleared. Missing or invalid legacy values normalise to `false` and the migration is idempotent (`raidState_` in Code.gs; covered by `tests/backend/stim-floor.test.js`). New saves never write the combined field.
...
`GITHUB_DATA_TOKEN`, `GITHUB_DATA_OWNER`, `GITHUB_DATA_REPO`, `GITHUB_DATA_BRANCH`, `GITHUB_DATA_PREFIX`, `GITHUB_DATA_MODE`. Keep `GITHUB_DATA_PREFIX` empty for the documented repository-root `state/` layout. For backward compatibility, a full-root value of `state` (or a scoped value ending in `/state`) resolves to that same single root and is never concatenated into `state/state/`. The token is read only at request time in `GitHubStore.gs` and is **never** returned, logged, put in an error message, committed, or exposed to the browser. Never place a real token or backup code in the repo or docs.

### Storage modes (`GITHUB_DATA_MODE`)

- **sheets** — bootstrap/recovery and maintenance mode. Migration tools can be previewed/executed/verified without switching.
...
> Production migration note (9 August 2026): GitHub mode was enabled after an eight-member migration and verification. A legacy production property set `GITHUB_DATA_PREFIX=state` while callers already supplied `state/...`, creating a newer duplicate tree under `state/state/...`. A protected cutover must mirror the live GitHub data to Sheets, switch to maintenance `sheets` mode, promote the exact latest duplicate dataset to canonical `state/...`, deploy the canonical-root guard, verify the canonical files, and only then return through `shadow` to `github`. The duplicate tree is retained only until canonical read/write acceptance succeeds, then removed in a separate recoverable Git commit.
