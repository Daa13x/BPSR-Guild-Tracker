...
Material migration: Google-account email identity and `google.script.run` are legacy baseline assumptions. `AuthApi.gs` introduces private character/PIN member, session and throttle tables plus JSON register/login/admin-login/logout/leaderboard routes. The real supplied guild logo was not available in the workspace and must be added unchanged at `assets/guild-logo.png` before deployment.
